...
my $PLOT_LINK2="http://mvic-biotech.na.pg.com/projects/Micro_Skin_Health/Bioinformatics_PH/plotpergene/";

my $PLOT_LINK="http://mvic-biotech.na.pg.com/projects/Micro_Skin_Health/Bioinformatics_PH/plotpergene/";
...
my $java_script_header="<script language=\"JavaScript\">
 ...
 <SCRIPT language=JavaScript src=\"http://artemis.na.pg.com/sorttable.js\"></SCRIPT>
";
...
####/,###
		   ...
		   if ($FILL ==1){
		       ...
		       $im2->filledEllipse($cor[0], $cor[1], 4*$cor[2], 4*$cor[2],$COLOR);
		   }else{
		       $im2->arc($cor[0], $cor[1], 4*$cor[2],4*$cor[2],0,360,$COLOR);
...
\n<tr><td><font color=\"#000099\">Acronym</font></td><td><font color=\"#000099\">AffyProbe</font></td><td><font color=\"#330033\">P</font></td><td><font color=\"#996600\">Fold</font></td><td><font color=\"#003300\">Description</font></td><td><font color=\"#000099\">KEGG_NAME</font></td>";
	...
	foreach my $name (sort keys %{$$geneh{$kegg_name}}){
	    ...
	    if (($P eq "N/A")&&($FC eq "N/A")&&($P eq "")&&($FC eq "")){next;}
	    $geneNumber++;
	    if ($FC>1){$UP++;}
	    if ($FC<1){$DOWN ++;}
	    if ($P<=0.05){
