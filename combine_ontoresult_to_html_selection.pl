...
my ($file, $CUT, $statpath, $longfile, $acrfile, $outname, $ontoidlist) = @ARGV;

my $usage = "$0 file_list P_CUT stat_path  annfile synfile outputname ontoidlist\n";
...
my %ontoid;
my $NN=0;
open (ONTO, "$ontoidlist")||die "could not open $ontoidlist\n";
while (my $line = <ONTO>){
    chomp $line;
    $NN++;
    $ontoid{$line}=$NN;
...
########we can use this to creat a gene page then I do not need to maintain the overall heatmap page, but just create one using the raw result file#####################   
   ...
   if ($line=~/(\S+)\_p\d+.result/){
	...
	$statfilename=$1;    
    }elsif ($line=~/(\S+)\_p\d+dn.result/){
	$statfilename=$1;
    }elsif ($line=~/(\S+)\_p\d+up.result/){
	$statfilename=$1;
    }elsif ($line=~/(\S+).result/){
	$statfilename=$1;
    }
...
#################################################
	...
	my @tmp=split /\t/, $M;
	if (@tmp<5){next;}
	if (defined $ontoid{$tmp[0]}){
	    my $goname=$ontoid{$tmp[0]}." ".$tmp[0]. " ". $tmp[4];
	    my $goshort=substr($goname, 0, 58);
	    my $id="<td>".$goshort."</td>";
	    my $p=$tmp[3];
	    my $n=$tmp[1];
	    my @genes=split /\#/, $tmp[5];
	    if ($p<=$CUT){
		if (!defined $good{$id}){
		    $good{$id}=1;
		}else{
	    ...
	    write_gene_html ($tmp[5], $pagename, "$goid -- $statfilename", \%PPP, \%FFF, \%long, \%acr);
	}
    }
    close C;
